...
class Coupons_el(BaserPage):
    ...
    def New_Coupons(self, element_data, new_coupons):
        public_li_key, public_li_value = new_coupons['public_li']
        ...
        self.send_key(locator=new_coupons['full_money'], locators=None, value=str(full_money))
        self.send_key(locator=new_coupons['subtract_money'], locators=None, value=str(subtract_money))
        ...
        if coupons_date_type[date_type_number].text == element_data['coupons_date_type']:
            # 固定日期的逻辑
            coupons_date_type[date_type_number].click()
            # 打开新建时间
        ...
        continue_restrict_lis = self.locator_element(locator=new_coupons['continue_restrict_lis'],
                                                     locators=new_coupons['public_li'])
        continue_restrict_number = random.randint(0, len(continue_restrict_lis) - 1)
        # 如果他选中指定分类
        if continue_restrict_lis[continue_restrict_number].text == element_data['continue_restrict']:
            ...
            appoint_lis = self.locator_element(locator=new_coupons['appoint_lis'], locators=new_coupons['public_li'])
